...
//  if(self->client)
//  {
//	self->client->pers.diehp = self->client->ps.stats[ STAT_MAX_HEALTH ];
//  }

  if( self && self->client && BG_InventoryContainsUpgrade( UP_BATTPACK, self->client->ps.stats ) )
  {
	  G_RadiusDamage(self->client->ps.origin, self, 200, 200, self, MOD_TARGET_LASER);
  }

//  if(meansOfDeath == MOD_TARGET_LASER && self->s.eType == ET_BUILDABLE)
//  {
//	  return;
//  }
  ...
  /*if(self->client && attacker->client)
  {
	  ...
	  {
		  if((attacker->client->ps.stats[ STAT_MAX_HEALTH ]/3) > self->client->pers.diehp)
		  {
			self->client->pers.diehp = attacker->client->ps.stats[ STAT_MAX_HEALTH ] / 3;
		  }
	  }
	  if(attacker->client->pers.teamSelection == PTE_HUMANS)
	  {
		  if(attacker->client->ps.stats[ STAT_MAX_HEALTH ] > self->client->pers.diehp)
		  {
			self->client->pers.diehp = attacker->client->ps.stats[ STAT_MAX_HEALTH ];
		  }
	  }
  }*/
...
if(attacker->client && self->client)
{
	...
	{
	//6  	Survivalist  	Dont die over 15 minutes
	if(attacker->client->pers.badges[ 6 ] != 1 && (attacker->client->pers.statscounters.timealive/60) >= 15)
	{
		G_WinBadge( attacker, 6 );
		attacker->client->pers.badgeupdate[6] = 1;
		attacker->client->pers.badges[6] = 1;
	}
	//2  	The Mad Granger      	Kill 1 Human wearing a Battle Suit with the Grange...
	if(attacker->client->pers.badges[ 2 ] != 1 
	&& (attacker->client->ps.weapon == WP_ABUILD || attacker->client->ps.weapon == WP_ABUILD2)
	...
	//5  	Blast it!  	Killed over 15 aliens by a blaster in a game
	if(attacker->client->pers.badges[ 5 ] != 1 && attacker->client->ps.weapon == WP_BLASTER
	&& meansOfDeath == MOD_BLASTER
	)
	{
		attacker->client->pers.blasterkills += 1;
		if(attacker->client->pers.blasterkills >= 15)
		{
	...
	//8  	Starcraft  	Kill one Tyrant While using a Rifle
	if(attacker->client->pers.badges[ 8 ] != 1 
	&& (attacker->client->ps.weapon == WP_MACHINEGUN)
	&& self->client->ps.weapon == WP_ALEVEL4 && meansOfDeath == MOD_MACHINEGUN)
	{
		G_WinBadge( attacker, 8 );
		attacker->client->pers.badgeupdate[8] = 1;
		attacker->client->pers.badges[8] = 1;
	}
	//9  	I Bite Pretty Hard  	kill a human with battlesuit on using a dretch
	if(attacker->client->pers.badges[ 9 ] != 1 
	&& meansOfDeath == MOD_LEVEL0_BITE
	&& BG_InventoryContainsUpgrade( UP_BATTLESUIT, self->client->ps.stats ))
	{
	...
	//11  	Spitter  	Kill a Human while using the barb of the granger
	if(attacker->client->pers.badges[ 11 ] != 1 
	&& (attacker->client->ps.weapon == WP_ABUILD2)
	...
	//14  	Gattling Gun  	Kill one Tyrant while using the chaingun
	if(attacker->client->pers.badges[ 14 ] != 1 
	&& (attacker->client->ps.weapon == WP_CHAINGUN)
	&& self->client->ps.weapon == WP_ALEVEL4 && meansOfDeath == MOD_CHAINGUN)
	{
		G_WinBadge( attacker, 14 );
		attacker->client->pers.badgeupdate[14] = 1;
		attacker->client->pers.badges[14] = 1;
	}
	//15  	Pain master  	Killed 5 aliens in a row with the psaw
	if(attacker->client->pers.sawrow > 0 && attacker->client->ps.weapon != WP_PAIN_SAW)
	{
		attacker->client->pers.sawrow = 0;
	}
	if(attacker->client->ps.weapon == WP_PAIN_SAW && meansOfDeath == MOD_PAINSAW)
	{
		attacker->client->pers.sawrow++;
	}
	
	if(attacker->client->pers.badges[ 15 ] != 1 
	&& (attacker->client->ps.weapon == WP_PAIN_SAW)
	&& attacker->client->pers.sawrow >= 5)
	{
	...
	//17  	Anti-Sniper  	kill 2 adv goon with your mass driver
	if(attacker->client->pers.mdadvgoonkills > 0 && attacker->client->ps.weapon != WP_MASS_DRIVER)
	{
		attacker->client->pers.mdadvgoonkills = 0;
	}
	if(attacker->client->ps.weapon == WP_MASS_DRIVER && self->client->ps.weapon == WP_ALEVEL3_UPG && meansOfDeath == MOD_MDRIVER)
	{
		attacker->client->pers.mdadvgoonkills++;
	}
	
	if(attacker->client->pers.badges[ 17 ] != 1 
	&& (attacker->client->ps.weapon == WP_MASS_DRIVER)
	&& attacker->client->pers.mdadvgoonkills >= 2)
	{
		G_WinBadge( attacker, 17 );
		attacker->client->pers.badgeupdate[17] = 1;
		attacker->client->pers.badges[17] = 1;
	}
	
	//18  	Tyrant's Might  	Charge and kill upto 3 humans in 1 Tyrant life
	if(attacker->client->pers.tyrantkills > 0 && attacker->client->ps.weapon != WP_ALEVEL4)
	{
		attacker->client->pers.tyrantkills = 0;
	}
	if(attacker->client->ps.weapon == WP_ALEVEL4 && meansOfDeath == MOD_LEVEL4_CHARGE)
	{
		attacker->client->pers.tyrantkills++;
	}
	
	if(attacker->client->pers.badges[ 18 ] != 1 
	&& (attacker->client->ps.weapon == WP_ALEVEL4 && meansOfDeath == MOD_LEVEL4_CHARGE)
	&& attacker->client->pers.tyrantkills >= 3)
	{
		G_WinBadge( attacker, 18 );
		attacker->client->pers.badgeupdate[18] = 1;
		attacker->client->pers.badges[18] = 1;
	}
	
	
	//20  	Electric Eel  	Killed 3 humans with using Zap
	if(attacker->client->pers.advmarakills > 0 && attacker->client->ps.weapon != WP_ALEVEL2_UPG)
	{
		attacker->client->pers.advmarakills = 0;
	}
	if(attacker->client->ps.weapon == WP_ALEVEL2_UPG && meansOfDeath == MOD_LEVEL2_ZAP)
	{
		attacker->client->pers.advmarakills++;
	}
	
	if(attacker->client->pers.badges[ 20 ] != 1 
	&& (attacker->client->ps.weapon == WP_ALEVEL2_UPG && meansOfDeath == MOD_LEVEL2_ZAP)
	&& attacker->client->pers.advmarakills >= 3)
	{
	...
	//21  	Attack Granger  	Kill 1 Human while using a granger
	if(attacker->client->pers.badges[ 21 ] != 1 
	&& (attacker->client->ps.weapon == WP_ABUILD || attacker->client->ps.weapon == WP_ABUILD2))
	{
	...
	//23  	Flying Dragoon  	Pounce and kill over 5 humans while being a dragoon...  	
	if(attacker->client->pers.pouncekills > 0 && attacker->client->ps.weapon != WP_ALEVEL3)
	{
		attacker->client->pers.pouncekills = 0;
	}
	if(attacker->client->ps.weapon == WP_ALEVEL3 && meansOfDeath == MOD_LEVEL3_POUNCE)
	{
		attacker->client->pers.pouncekills++;
	}
	
	if(attacker->client->pers.badges[ 23 ] != 1 
	&& (attacker->client->ps.weapon == WP_ALEVEL3 && meansOfDeath == MOD_LEVEL3_POUNCE)
	&& attacker->client->pers.pouncekills >= 5)
	{
		G_WinBadge( attacker, 23 );
		attacker->client->pers.badgeupdate[23] = 1;
		attacker->client->pers.badges[23] = 1;
	}
	//24  	Hugs and Swipings  	kill 2 Humans while using a basilisk on 1 life
	if(attacker->client->pers.liskkills > 0 && attacker->client->ps.weapon != WP_ALEVEL1)
	{
		attacker->client->pers.liskkills = 0;
	}
	if(attacker->client->ps.weapon == WP_ALEVEL1)
	{
		attacker->client->pers.liskkills++;
	}
	
	if(attacker->client->pers.badges[ 24 ] != 1 
	&& (attacker->client->ps.weapon == WP_ALEVEL1)
	&& attacker->client->pers.liskkills >= 2)
	{
		G_WinBadge( attacker, 24 );
		attacker->client->pers.badgeupdate[24] = 1;
		attacker->client->pers.badges[24] = 1;
	}
	//Delete   	27  	David and Goliath  	Kill a tyrant the lasgun  	2
	if(attacker->client->pers.badges[ 27 ] != 1 
	&& (attacker->client->ps.weapon == WP_LAS_GUN && meansOfDeath == MOD_LASGUN)
	&& self->client->ps.weapon == WP_ALEVEL4)
	{
		G_WinBadge( attacker, 27 );
		attacker->client->pers.badgeupdate[27] = 1;
		attacker->client->pers.badges[27] = 1;
	}
	//31  	Red right hand-y weapon  	Kill a goon in one fullshot
	if(attacker->client->pers.badges[ 31 ] != 1 
	&& (attacker->client->ps.weapon == WP_LUCIFER_CANNON)
	&& meansOfDeath == MOD_LCANNON 
	)
	{
	...
	//32 Stealth Killer  	killed 10 enemy's while on invisible mode.
	if(attacker->client->pers.badges[ 32 ] != 1 )
	{
		if(G_isPlayerConnected(attacker) && ( attacker->client->ps.eFlags & EF_NODRAW ))
		{
			attacker->client->pers.stealthkills++;
		}
		if(attacker->client->pers.stealthkills >= 10)
		{
	...
	//33  	Divine Gift  	Killed 5 Enemy while using God Mode
	if(attacker->client->pers.badges[ 33 ] != 1 )
	{
		if(( attacker->flags & FL_GODMODE ) )
		{
			attacker->client->pers.godkills++;
		}
		if(attacker->client->pers.godkills >= 5)
		{
	...
	//34  	Pro Nader  	kill 5 aliens with 1 grenade
	if(attacker->client->pers.badges[ 34 ] != 1 && meansOfDeath == MOD_GRENADE 
	&& attacker->client->pers.teamSelection == PTE_HUMANS)
	{
		if(level.time > attacker->client->pers.explosiontimer)
		{
			attacker->client->pers.explosionkills=0;
		}
		attacker->client->pers.explosionkills++;
		if(attacker->client->pers.explosionkills == 1)
		{
			attacker->client->pers.explosiontimer = level.time + 1500;
		}
		if(attacker->client->pers.explosionkills >=5)
		{
	...
	//35  	Suicide Bomber  	kill 3 humans in 1 dretch nade
	if(attacker->client->pers.badges[ 35 ] != 1 && meansOfDeath == MOD_GRENADE
	&& attacker->client->ps.weapon == WP_ALEVEL0)
	{
		if(level.time > attacker->client->pers.explosiontimer)
		{
			attacker->client->pers.explosionkills=0;
		...
		if(attacker->client->pers.explosionkills >=3)
		{
			G_WinBadge( attacker, 35 );
			attacker->client->pers.badgeupdate[35] = 1;
			attacker->client->pers.badges[35] = 1;
		}
	}
	
	//36  	Poison Freak  	kill 3 Humans by a Poisoned Icewave
	if(attacker->client->pers.badges[ 36 ] != 1 && meansOfDeath == MOD_POISON
	&& attacker->client->ps.weapon == WP_ALEVEL0
	//&& self->client->ps.stats[ STAT_STATE ] & SS_ICEWAVED //This is what fail?
	)
	{
		if(level.time > attacker->client->pers.explosiontimer)
		{
		...
		if(attacker->client->pers.explosionkills == 1)
		{
			attacker->client->pers.explosiontimer = level.time + 10000;
		}
		if(attacker->client->pers.explosionkills >=3)
		{
	...
	//37  	Fire Breathing  	Human kill 10 Dretches in 1 Flamethrower without r...
	if(attacker->client->pers.badges[ 36 ] != 1 
	&& (meansOfDeath == MOD_FLAMER || meansOfDeath == MOD_FLAMER_SPLASH)
	)
	{
		attacker->client->pers.flamerkills++;
		
		if(attacker->client->pers.flamerkills >=10)
		{
  ...
  if( attacker )
  {
    ...
    if( attacker->client )
    {
      ...
      //For the newbie bonus.
      if( attacker != self && attacker->client->ps.stats[ STAT_PTEAM ]  != self->client->ps.stats[ STAT_PTEAM ] ) 
      {
				if(self->newbiebonus != 1)
				{
					self->deadcounter++;
				}
				//Attacker shit
				/*if(attacker->newbiebonus == 1)
				{
					attacker->killcounter++;
					if(attacker->killcounter++ >= 3)
					{
						attacker->newbiebonus = 0;
						attacker->killcounter = 0;
						trap_SendServerCommand( attacker-g_entities, va("print \"^3Newbie Bonus ^7OFF\n\"" ) );
					}
				}
				else
				{
					attacker->deadcounter = 0;
				}*/
				attacker->deadcounter = 0;
				if(self->deadcounter >= 2)
				{
					//self->newbiebonus = 1;
					self->killcounter = 0;
					self->deadcounter = 0;
					
					trap_SendServerCommand( self-g_entities, va("print \"^3Newbie Bonus!, ^5Free Credits.\n\"" ) );	
					//43  	Newbie  	Have won the the newbie bonus for first time
					if(self->client->pers.badges[ 43 ] != 1)
					{
						self->client->pers.badgeupdate[43] = 1;
						self->client->pers.badges[43] = 1;
						G_WinBadge( self, 43 );
					}
					if(self->client->pers.teamSelection == PTE_HUMANS) 
						self->client->ps.persistant[ PERS_CREDIT ] = HUMAN_MAX_CREDITS;
						
					if(self->client->pers.teamSelection == PTE_ALIENS) 
						self->client->ps.persistant[ PERS_CREDIT ] = ALIEN_MAX_KILLS;
				}
			}
			//Ends newbie bonus
  ...
  if( !tk && meansOfDeath != MOD_TARGET_LASER)
  {
  ...
  else if( attacker && attacker->client && meansOfDeath != MOD_TARGET_LASER)
  {
  ...
  {
	  trap_SendServerCommand( -1,
	       va( "print \"%s^7 was electrocuted by %s^7 battpack\n\"",
	       self->client->pers.netname, attacker->client->pers.netname));
	     G_LogOnlyPrintf("%s^7 was electrocuted by %s^7 battpack\n",
	       self->client->pers.netname, attacker->client->pers.netname );
  }
  ...
  if( attacker && attacker->client )
  {
    ...
    if( g_killerHP.integer ||
        ( g_devmapKillerHP.integer && g_cheats.integer ) )
     {
        if(meansOfDeath != MOD_UNKNOWN)
            trap_SendServerCommand( self-g_entities, va( "print \"^7Your killer, %s^7, had %3i HP.\n\"", killerName, attacker->health ) );
      }
    ...
    if( attacker == self || OnSameTeam( self, attacker ) )
    {
      ...
      {
        AddScore( attacker, 1 );
        attacker->client->ps.persistant[ PERS_CREDIT ] -= 1;
      }
    ...
    else
    {
      ...
      if(attacker->client->pers.energy < 1000)
      {
				attacker->client->pers.energy += 200;
				if(attacker->client->pers.energy >= 1000)
				{
					trap_SendServerCommand( attacker-g_entities, va(
        "print \"^5You Have Full Energy type !energy for avaliable powers\n\"" ) );
				}
			}
      if( attacker->client->pers.teamSelection == PTE_ALIENS ) 
      {
    ...
    if( self->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
    {
      ...
      for( i = 0; i < MAX_CLIENTS; i++ )
      {
        ...
        if( percentDamage > 0 && percentDamage < 1)
        {
          ...
          if(player->client->pers.energy < 1000)
					{
						player->client->pers.energy += 200;
						if(player->client->pers.energy >= 1000)
						{
...
static float G_CalcDamageModifier( vec3_t point, gentity_t *targ, gentity_t *attacker, int class, int dflags )
{
  ...
  else
  {
    ...
    if( attacker && attacker->client && modifier == 2 )
    {
      ...
      if(attacker->client->pers.energy < 1000)
      {
				attacker->client->pers.energy += 50;
				if(attacker->client->pers.energy >= 1000)
				{
					trap_SendServerCommand( attacker-g_entities, va(
        "print \"^5You Have Full Energy type !energy for avaliable powers\n\"" ) );
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  if(attacker && attacker->client && attacker->client->pers.handicap)
	  return;
  if( !targ->takedamage )
    return;

  if(targ && targ->client && targ->client->pers.handicap)
	  damage = damage * 2;

	/*if((mod == MOD_FLAMER || mod == MOD_FLAMER_SPLASH) && targ->client == attacker->client) // && targ == attacker)
		{
			trap_SendServerCommand( -1, va( "print \"" S_COLOR_WHITE " g_dmg smg\n\"" ) );
			return;
		}*/
  ...
  if( !attacker )
    attacker = &g_entities[ ENTITYNUM_WORLD ];
		...
		if( attacker->flags & FL_GODMODE && targ->s.eType == ET_BUILDABLE)
   {
     return;
   }
  ...
  if(inflictor && targ && inflictor->client && inflictor->client->pers.doubledmg == 1 && targ->client)
      {
                        damage = (int)(damage *  1.40); //1.45 humanos ganan, 1.35 aliens ganan lets try 1.40 :p
      }
    ...
    if( attacker && attacker->client )
    {
      ...
      else if( targ->s.eType == ET_BUILDABLE )
      {
        ...
        if( targ->health > 0 && ( targ->health - take ) <=0 )
        {
          ...
          if(attacker->client->pers.energy < 1000)
					{
						attacker->client->pers.energy += 100;
						if(attacker->client->pers.energy >= 1000)
						{
							trap_SendServerCommand( attacker-g_entities, va(
							"print \"^5You Have Full Energy type !energy for avaliable powers\n\"" ) );
                         ...
                         float radius, gentity_t *ignore, int mod )
{
  ...
  for( e = 0; e < numListedEntities; e++ )
  {
    ...
    //For some reason the flamer dont work right when is supposed to work for example aiming at the base of a egg, acid etc would not kill it :s
    if((mod == MOD_FLAMER || mod == MOD_FLAMER_SPLASH) && ent->client == attacker->client) // && targ == attacker)
		{
			//trap_SendServerCommand( -1, va( "print \"" S_COLOR_WHITE " g_radius smg\n\"" ) );
			continue;
		}
    ...
    if(!ent->client && attacker->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_LEVEL0 && mod == MOD_GRENADE)
    {
			//trap_SendServerCommand( attacker-g_entities, "print \"^5Overmind: ^7Dretche explosion touched a build.\n\"" );
			damage = damage / 2;
		}
