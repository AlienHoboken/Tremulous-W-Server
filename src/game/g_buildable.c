...
void ASpawn_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
  ...
  new->ID = ( ++level.lastBuildID > MAX_BUILD_LOG ) ? ( level.lastBuildID = 1 ) : level.lastBuildID;
  ...
  if( attacker && attacker->client )
  {
    ...
    G_LogPrintf( "Decon: %i %i %i: %s^7 destroyed %s by %s\n",
      ...
      modNames[ mod ] );
	//22  	I like my eggs...scrambled  	Kill 3 eggs with a saw within 2 minutes
	if(attacker->client->ps.weapon == WP_PAIN_SAW  && self->s.modelindex == BA_A_SPAWN)
	{
		if(level.time > attacker->client->pers.saweggstimer)
		{
			attacker->client->pers.saweggscounter = 0;
		}
		if(attacker->client->pers.saweggscounter < 1)
		{
			attacker->client->pers.saweggstimer = level.time + 90000;
		}
		
		attacker->client->pers.saweggscounter++;
		
	}
	if(attacker->client->pers.badges[ 22 ] != 1 
	&& ( (attacker->client->ps.weapon == WP_PAIN_SAW ) && self->s.modelindex == BA_A_SPAWN) && attacker->client->pers.saweggscounter >= 3 )
	{
...
void HSpawn_Die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
  ...
  if( attacker && attacker->client )
  {
    ...
    G_LogPrintf( "Decon: %i %i %i: %s^7 destroyed %s by %s\n",
      ...
      //7  	Ultra Granger  	Destroy The Reactor While being a Granger
	if(attacker->client->pers.badges[ 7 ] != 1 
	&& ( (attacker->client->ps.weapon == WP_ABUILD2) && self->s.modelindex == BA_H_REACTOR) )
	{
			attacker->client->pers.badgeupdate[7] = 1;
			attacker->client->pers.badges[7] = 1;
			G_WinBadge( attacker, 7 );
	}
	//19  	Specialist  	Kill the reactor by the goons barbs
	if(attacker->client->pers.badges[ 19 ] != 1 
	&& ( (attacker->client->ps.weapon == WP_ALEVEL3_UPG) && self->s.modelindex == BA_H_REACTOR) && mod == MOD_LEVEL3_BOUNCEBALL )
	{
			attacker->client->pers.badgeupdate[19] = 1;
			attacker->client->pers.badges[19] = 1;
			G_WinBadge( attacker, 19 );
	}
...
void G_BuildableThink( gentity_t *ent, int msec )
{
  ...
  if(ent->s.modelindex == BA_H_REACTOR)
	  bTime = g_reactorBuildTime.integer;
...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin )
{
  ...
  //Lets check builds per area

  int       j,reactorclose,omclose;
  gentity_t *ent2;
  int       distance2 = 0;
  int       minDistance = 500;
  int       structuresclose = 0;
  vec3_t    temp_v;
  ...
  if( (!( normal[ 2 ] >= minNormal || ( invert && normal[ 2 ] <= -minNormal ) )) && buildable == BA_A_HOVEL)
    return IBE_NORMAL;
  ...
  if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
  {
    ...
    //Limit the number of eggs to 20
    if(buildable == BA_A_SPAWN && G_countStructures(BA_A_SPAWN) >= 16)
    {
    	return IBE_MAXEGGS;
    }
  ...
  {
	 reactorclose = omclose = 0;
    for ( j = 1, ent2 = g_entities + j; j < level.num_entities; j++, ent2++ )
    {
      //This can certainly lag the server..
        if( ent2->s.eType != ET_BUILDABLE )
            continue;

        if(ent->client->ps.stats[ STAT_PTEAM ] != BG_FindTeamForBuildable( ent2->s.modelindex ))
            continue;
        //if entity is a power item calculate the distance to it
          VectorSubtract( entity_origin, ent2->s.origin, temp_v );
          distance2 = VectorLength( temp_v );

          if( distance2 < (minDistance+200) ) {
            //We have a structure in the range
            if( ent2->s.modelindex == BA_H_REACTOR)
            {
				reactorclose = 1;
				//structuresclose = 0;
				//break;
			}
			if(ent2->s.modelindex == BA_A_OVERMIND)
			{
				omclose = 1;
			}
            structuresclose++;
              /*if(structuresclose > 6)
              {
                  //Its better this overload than keep running the for and calculating all other shit, just saying.
                  break;
              }*/
          }
      }

    if(structuresclose > 4 && (reactorclose == 0 && omclose == 0))
    {
      //trap_SendServerCommand( ent-g_entities, "print \"^5Reactor: ^1 You cant build more structures in this area.\n\"" );
      //ent->client->ps.stats[ STAT_MISC ] +=BG_FindBuildDelayForWeapon( ent->s.weapon ) * 2;
      return IBE_STRUCTURES;
    }
    if(reactorclose == 1 && structuresclose > 25)
    {
		return IBE_STRUCTURES;
	}
	if(omclose == 1 && structuresclose > 50)
    {
		return IBE_STRUCTURES;
	}
  }
...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  ...
  if( builder->client ) 
  {
     ...
     if(builder->client->pers.energy < 1000)
      {
				builder->client->pers.energy += 50;
				if(builder->client->pers.energy >= 1000)
				{
					trap_SendServerCommand( builder-g_entities, va(
...
qboolean G_BuildIfValid( gentity_t *ent, buildable_t buildable )
{
  ...
  switch( G_CanBuild( ent, buildable, dist, origin ) )
  {
    ...
    case IBE_MAXEGGS:
    	trap_SendServerCommand( ent-g_entities, va( "print \"^316 eggs max, build some defense.\n\"" ) );
    return qfalse;
    ...
    case IBE_RPTWARN2:
      ...
      case IBE_STRUCTURES:
          if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
            ...
            trap_SendServerCommand( ent-g_entities, "print \"^5Overmind: ^1 You cant build more structures in this area.\n\"" );
...
void G_BaseSelfDestruct( pTeam_t team )
{
 ...
 int G_countStructures(buildable_t buildableType)
 {
	 gentity_t *tempent;
	 int i;
	 int numBuildables = 0;
	 ...
	 for ( i = 1, tempent = g_entities + i; i < level.num_entities; i++, tempent++ )
	 {
	 	if( tempent->s.eType != ET_BUILDABLE )
	 		continue;
	 	if( tempent->s.modelindex == buildableType && tempent->spawned && tempent->health > 0 )
	         numBuildables++;
	 }
	 return numBuildables;
 }
