...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"namelog", G_admin_namelog, "k",
      "display a list of names and IPs used by recently connected players",
      "(^5name^7)"
    },
...
{"unforcespec", G_admin_unforcespec, "s",
...
//   },
   {"bank", G_bank, "h", 
     "Show your banked credits",
     ""   
   },   
   {"w", G_w, "h", 
     "Allow you withdraw credits from your !bank",
     "Credits"
   },   
   {"d", G_d, "h", 
     "Allow you deposit credits into your !bank",
     "Credits"
   },
   {"energy", G_energy, "h", 
     "Shows your energy level and usable powers",
     "energy"
   },
   {"heal", G_heal, "h",
      "Heal yourself Cost ^51000 ^7Energy Points",
      ""
   },
   {"testvars", G_testvars, "q",
      "plox",
      ""
   },
   {"badges", G_badges, "h",
   ...
   {"badgelist", G_badgelist, "h",
      "List Badges",
      ""
   },
   {"listbadges", G_badgelist, "h",
      "List Badges",
      ""
   },
   {"lol", G_lol, "q",
   ...
   {"allowdeconpower", G_allowdeconpower, "e",
      "Dont use this command.",
      ""
   },
//   {"doubledmg", G_doubledmg, "h",
//      "Your weapon will have double dmg. Cost ^51000XP",
//      ""
//   },
   {"test", G_test, "q",
         "This command is for developing only, so dont use it",
         ""
   },
   {
	"loadmodel", G_loadModel, "q", "Load a model into the game, model vanish in 10 seconds",
		""
   },
   {"gdelete", G_adminDeleteGlobal, "s",
         "delete a global entry",
         "(^5 global id ^7)"
   },
   {"glist", G_adminListGlobals, "s",
         "display a list of active globals",
         ""
   },
   {"gmute", G_adminGlobalMute, "s",
         "global mute",
   ...
   {"gforcespec", G_adminGlobalForcespec, "s",
   ...
   {"gdenybuild", G_adminGlobalDenyBuild, "s",
   ...
   {"ghandicap", G_adminGlobalHandicap, "s",
...
//
//qboolean G_rtd( gentity_t *ent, int skiparg )
//{
//	char map[ MAX_CVAR_VALUE_STRING ] = {""};
//	gentity_t *ent2;
//
//	int seed, random_integer, randomgod, min, sec, i, ispowered;
//	int pids[ MAX_CLIENTS ];
//	/*This is to give different gifts if two players do !rtd in the same ms.*/
//
//	trap_Cvar_VariableStringBuffer( "mapname", map, sizeof( map ) );
//
//	if((ent->client->pers.rtdtime > level.time))
//	{
//		min = ((ent->client->pers.rtdtime - level.time)/1000) / 60 ;
//		sec = ((ent->client->pers.rtdtime - level.time)/1000) % 60 ;
//		if(sec < 10)
//		{
//			trap_SendServerCommand( ent-g_entities, va(
//        "print \"^1You must wait %d:0%d\n\"",min,sec ) );
//				return qfalse;
//		}
//		else
//		{
//			trap_SendServerCommand( ent-g_entities, va(
//        "print \"^1You must wait %d:%d\n\"",min,sec ) );
//				return qfalse;
//		}
//	}
...
//		{
//			trap_SendServerCommand( ent-g_entities, va(
//        "print \"^1Cant use !rtd in the first 10 minuts of this map.\n\"" ) );
//				return qfalse;
//		}
...
//	{
//		//RTD WONT WORK WITHOUT RC OR OM
//		ispowered = 0;
...
//		{
//			trap_SendServerCommand( ent-g_entities, va(
//        "print \"^1You need a overmind/rc to use rtd.\n\"" ) );
//				return qfalse;
//		}
//	}
//	if(ent->health <= 0)
...
qboolean G_admin_stfu( gentity_t *ent, int skiparg )
{
  int pids[ MAX_CLIENTS ];
  char name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
  char command[ MAX_ADMIN_CMD_LEN ], *cmd;
  ...
  if( !admin_higher( ent, &g_entities[ pids[ 0 ] ] ) )
  {
    ADMP( va( "^3!%s: ^7Wtf noob he is a higher admin.\n", cmd ) );
    return qfalse;
  }
  vic = &g_entities[ pids[ 0 ] ];
  if( vic->client->pers.stfu == qtrue )
  {
    if( !Q_stricmp( cmd, "stfu" ) )
    {
      ADMP( "^3!mute: ^7player is already stfu\n" );
      return qtrue;
    }
    vic->client->pers.stfu = qfalse;
    /*CPx( pids[ 0 ], "cp \"^1You have been unmuted\"" );*/
    AP( va( "print \"^3!unstfu: ^7%s^7 has been unstfu by %s\n\"",
            vic->client->pers.netname,
            ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }
  else
  {
    ...
    {
      ADMP( "^3!unmute: ^7player is not currently stfu\n" );
      return qtrue;
    }
    vic->client->pers.stfu = qtrue;
    /*CPx( pids[ 0 ], "cp \"^1You've been muted\"" );*/
    /*AP( va( "print \"^3!mute: ^7%s^7 has been muted by ^7%s\n\"",
            vic->client->pers.netname,
            ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );*/
...
qboolean G_admin_register(gentity_t *ent, int skiparg ){
  ...
  if( !ent ) return qtrue;
	...
	if(ent->client->pers.rtimer >= level.time)
	{
		ADMP( va( "^3!register: ^7 Wait %d Seconds.\n", (ent->client->pers.rtimer - level.time) / 1000  ) );
		return qfalse;
	}
	ent->client->pers.rtimer = level.time + 60000;
	
  levela = G_admin_level(ent);
  ...
  {
    ADMP( va( "^3!register: ^7 Update your client. ^3\nInstructions: http://blogwtf.com/backport.\n" ) );
    return qfalse;
  }
  ...
  {
		if( trap_mysql_runquery( va("UPDATE players SET name=\"%s\" WHERE id=\"%d\" LIMIT 1", ent->client->pers.netname, ent->client->pers.mysqlid ) ) == qtrue )
		{
			ADMP( va( "^5Personal Web: ^7 http://blogwtf.com/player/%d\n", ent->client->pers.mysqlid) );
			trap_mysql_finishquery();
		}
		else
		{
			ADMP( va( "^3!register: ^7 Error happened when upading the database.\n" ) );
			trap_mysql_finishquery();
			return qfalse;
		}
		//trap_mysql_finishquery();
	...
	{
		ADMP( va( "^3!register: ^7 No Database Connection.\n" ) );
	}
	
  trap_SendConsoleCommand( EXEC_APPEND,va( "!setlevel %d %d;",ent - g_entities, levela) );
...
qboolean G_bank( gentity_t *ent, int skiparg )
{
    if(strcmp(ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") == 0)
    {  
        trap_SendServerCommand( ent-g_entities, va(
        "print \"^7You need update your tremulous, do it now and win ^510000^7 human Credits and ^5100^7 alien evos.\n^3Download from: ^5http://blogwtf.com/backport\n\"" ) );
        return qtrue;
    }
    if(level.time - ent->client->pers.banktime < 2000)
    {
        ADMP( "^3You can use bank only every 2 seconds\n" );
			return qfalse;
    }
    if(ent->client->pers.mysqlid <= 0)
    {
			trap_SendServerCommand( ent-g_entities, va(
        "print \"^1Please execute !sync, if that dont work, report the error to ^2http://blogwtf.com/forums\n\"" ) );
        return qfalse;
		}
		if(level.time - ent->client->pers.banktime < 2000)
    {
    ...
    {
        ent->client->pers.banktime = level.time;
    }
    if( ent->client->pers.credits >= 0 && ent->client->pers.evos >= 0 )
    {
        trap_SendServerCommand( ent-g_entities, va(
        "print \"^5Credits: ^7$%d \n^1Evos: ^7$%d\n\"",ent->client->pers.credits,ent->client->pers.evos) );
        
        //42  	Banker  	Used bank for first time
		if(ent->client->pers.badges[ 42 ] != 1)
		{
			ent->client->pers.badgeupdate[42] = 1;
			ent->client->pers.badges[42] = 1;
		}
        return qtrue;
    }
    else
...
qboolean G_w( gentity_t *ent, int skiparg )
{
//  int minargc;
//  int credits,dbcredits;
//  char amnt_chr[11];
//  int max_creds;
//  char data[255];

	trap_SendServerCommand( ent-g_entities, va(
        "print \"^1You cant withdraw for now.\n\"" ) );
				return qfalse;
...
//    if(level.time - level.startTime < 600000 )
//    {
//        ADMP( "^3You can use withdraw after minut 10\n" );
//				return qfalse;
//    }
//
//		if( ent->client->pers.credits < 0 || ent->client->pers.evos < 0 )
//    {
//        trap_SendServerCommand( ent-g_entities, va("print \"^1Bank is Off\"" ) );
//        return qfalse;
//    }
//
//    if(level.time - ent->client->pers.banktime < 2000)
...
//                return qfalse;
//  }
//  G_SayArgv( 1 + skiparg, amnt_chr, sizeof( amnt_chr ) );
//  //convert the text into a int
//  credits = atoi(amnt_chr);
//  if(credits < 1 )
//  {
//      /*trap_SendServerCommand( ent-g_entities, va(
//        "print \"^1You can't withdraw < 1 :p\n\"" ) );*/
//                return qfalse;
//  }
//  //Checar cuantos creditos tiene el usuario
//  if (ent->client->pers.teamSelection == PTE_HUMANS )
//  {
//				max_creds = HUMAN_MAX_CREDITS;
//        if(credits > HUMAN_MAX_CREDITS)
//        {
//            credits = HUMAN_MAX_CREDITS;
//        }
//        if(credits > HUMAN_MAX_CREDITS - ent->client->ps.persistant[ PERS_CREDIT ])
//        {
//            credits = HUMAN_MAX_CREDITS - ent->client->ps.persistant[ PERS_CREDIT ];
//        }
//        dbcredits = ent->client->pers.credits;
//  }
//  else
//  {
//        max_creds = ALIEN_MAX_KILLS;
//        if(credits > ALIEN_MAX_KILLS)
//        {
//            credits = ALIEN_MAX_KILLS;
//        }
//        if(credits > ALIEN_MAX_KILLS - ent->client->ps.persistant[ PERS_CREDIT ])
//        {
//            credits = ALIEN_MAX_KILLS - ent->client->ps.persistant[ PERS_CREDIT ];
//        }
//        dbcredits = ent->client->pers.evos;
//  }
//  if(credits > dbcredits)
//  {
//     credits = dbcredits;
//  }
//  if(credits <= 0)
...
//   else
//   {
//        //If theyr passed all checks lets get the money from mysql
//        if(ent->client->pers.teamSelection == PTE_HUMANS)
//        {
//								ent->client->pers.credits -= credits;
//                trap_SendServerCommand( ent-g_entities, va("print \"^1You Withdrew $%d \"",credits ) );
//        }
//        else
//        {
//								ent->client->pers.evos -= credits;
//								trap_SendServerCommand( ent-g_entities, va("print \"^1You Withdrew $%d \"",credits ) );
//        }
...
qboolean G_lol(gentity_t *ent, int skiparg)
{
	int i;
	gentity_t *ent2;
	vec3_t    dir2;
	if(level.lol < 1)
	{
		...
		for( i = MAX_CLIENTS; i < level.num_entities; i++ )
		{
			...
			if(!ent2->client || ent2->health < 1)
			{
				continue;
			}
				
				/*G_AddPredictableEvent( ent2, EV_ALIEN_EVOLVE, 0 );
				dir2[0] = 0;
				dir2[1] = 1;
				dir2[2] = 0;
				G_AddPredictableEvent( ent2, EV_ALIEN_ACIDTUBE, DirToByte( dir2 )  );*/
				G_AddEvent( ent2, EV_MEDKIT_USED, 0 );
		}
	}
	else
	{
		...
		for( i = MAX_CLIENTS; i < level.num_entities; i++ )
		{
			...
			if(!ent2->client || ent2->health < 1)
			{
				...
				G_AddPredictableEvent( ent2, EV_HUMAN_BUILDABLE_EXPLOSION, 0 );
				dir2[0] = 1;
				dir2[1] = 0;
				dir2[2] = 0;
				G_AddPredictableEvent( ent2, EV_ALIEN_ACIDTUBE, DirToByte( dir2 )  );
...
qboolean G_test(gentity_t *ent, int skiparg )
{
	//gentity_t *tent;

	//ent->s.constantLight = r | ( g << 8 ) | ( b << 16 ) | ( i << 24 );
	//ent->s.eType = ET_LIGHTFLARE;

//	ent->classname = "humanCorpse";
//	ent->s.eFlags = EF_DEAD;
//	ent->s.eType = ET_CORPSE;
	//G_AddPredictableEvent( ent, EV_TESLATRAIL, 0 );// <-- very good, problem... cant be removed..
//
//	      // send railgun beam effect
//	        tent = G_TempEntity( ent->s.origin, EV_TESLATRAIL );
//
//	        VectorCopy( ent->s.origin, tent->s.origin2 );
//
//	        tent->s.generic1 = tent->s.number; //src
//	        tent->s.clientNum =  ent->s.number; //dest
//	tent = god_effect(ent);
	SpawnCorpse( ent );
	trap_SendServerCommand( ent-g_entities, va(
	        "print \"^1test exectued bro\n\"" ) );
	return qtrue;
}

qboolean G_loadModel(gentity_t *ent, int skiparg) {
	gentity_t *model;
	...
	if (cmd && *cmd == '!')
		cmd++;
	if (G_SayArgc() < 2 + skiparg) {
		ADMP( va( "^3!%s: ^7usage: !%s models/.../.../...md3\n", cmd, cmd ) );
		return qfalse;
	}
	G_SayArgv(1 + skiparg, filename, sizeof(filename));
	if(!(modelindex=G_ModelIndex(filename)))
	{
		ADMP("Cant load this model\n");
		return qfalse;
	}

	model = G_Spawn();
	model->parent = ent;
	model->s.modelindex = modelindex;
	//		base->model = "models/weapons/rifle/rifle.md3";
	//		base->s.modelindex2 = G_ModelIndex("models/weapons/rifle/rifle.md3");
	G_SetOrigin(model, ent->s.origin); // sets where the turret is
	model->think = G_modelThink;
	model->nextthink = level.time + 5000;
	VectorSet( model->r.mins, -15, -15, -20 );
	VectorSet( model->r.maxs, 35, 15, -5);

	trap_LinkEntity(model);
	return qtrue;
}

qboolean G_allowdeconpower( gentity_t *ent, int skiparg )
{
  int pids[ MAX_CLIENTS ];
  char name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
  char command[ MAX_ADMIN_CMD_LEN ], *cmd;
  ...
  vic = &g_entities[ pids[ 0 ] ];
  if( vic->client->pers.allowdeconpower )
  {
    ADMP( "^3!Allowdeconpower: ^7player have allready permissions to decon power.\n" );
    return qtrue;
  }
  else
  {
	  //trap_SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " can decon power now\n\"", vic->client->pers.netname ) );
	  vic->client->pers.allowdeconpower = qtrue;
	  CPx( pids[ 0 ], "cp \"^1You can decon now.\"" );
  }
  ClientUserinfoChanged( pids[ 0 ] );
  return qtrue;
}
qboolean G_d( gentity_t *ent, int skiparg )
{
  int minargc;
  int credits;
  char amnt_chr[11];
	...
	if( ent->client->pers.credits < 0 || ent->client->pers.evos < 0 )
  {
	...
	if(level.waveactivated > 0)
	{
		trap_SendServerCommand( ent-g_entities, va(
        "print \"^1Cannot use !d on waves time. \n\"" ) );
				return qfalse;
	}
  ...
  minargc = 2 + skiparg;
    if( G_SayArgc() < minargc )
  {
    ADMP( "^3!d: ^7usage: !d amount\n" );
    return qfalse;
 }
  if(strcmp(ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") == 0)
  {
    trap_SendServerCommand( ent-g_entities, va(
        "print \"^7You need update your tremulous, do it now and win ^510000^7 human Credits and ^5100^7 alien evos.\n^3Download: ^5http://blogwtf.com/backport\n\"" ) );
           return qfalse;
  }
  ...
  G_SayArgv( 1 + skiparg, amnt_chr, sizeof( amnt_chr ) );
  credits = atoi(amnt_chr);
  if(credits > ent->client->ps.persistant[ PERS_CREDIT ])
  {
		credits = ent->client->ps.persistant[ PERS_CREDIT ];
	}
  if(credits < 1 )
  {
      /*trap_SendServerCommand( ent-g_entities, va(
  ...
  if(ent->client->pers.teamSelection == PTE_HUMANS)
  {
    trap_SendServerCommand( ent-g_entities, va("print \"^1You Deposited $%d \"",credits ) );
  }
  else
  {
     trap_SendServerCommand( ent-g_entities, va("print \"^1You Deposited $%d \"",credits ) );
...
qboolean G_energy( gentity_t *ent, int skiparg )
{
	if(!ent || !ent->client || ( ent->client->pers.teamSelection == PTE_NONE ))
  {
        trap_SendServerCommand( ent-g_entities, va(
  ...
  trap_SendServerCommand( ent-g_entities, va("print \"^7Energy: ^5%d ^7 Commands: ^3!heal: ^51000^7^7\n\"",ent->client->pers.energy ) );
  return qtrue;
}
...
qboolean G_heal( gentity_t *ent, int skiparg )
{
    ...
    if(ent->client->pers.energy >= 1000) 
    {    
        ent->client->pers.energy -= 1000; 
    ...
    {    
        trap_SendServerCommand( ent-g_entities, va(
            "print \"^7You need ^51000 ^7Energy Points\n\"" ) ); 
    }    
    return qfalse;
}
...
//{
//	trap_SendServerCommand( ent-g_entities, va(
//	        "print \"^1Deactivated for testing\n\"" ) );
//	                                return qfalse;
//	return qtrue;
//        if(ent->client->pers.teamSelection != PTE_HUMANS
//        && ent->client->pers.teamSelection != PTE_ALIENS )
...
//        {
//                trap_SendServerCommand( ent-g_entities, va(
//                        "print \"^7You have double dmg allready\n\"" ) );
//                return qfalse;
//        }
//        if(ent->client->pers.energy >= 1000)
//        {
...
//                trap_SendServerCommand( ent-g_entities, va(
//                        "print \"^2You got double dmg!\n\"" ) );
//                return qtrue;
//        }
//        else
//        {
//                trap_SendServerCommand( ent-g_entities, va(
//                "print \"^7You need ^51000XP\n\"" ) );
//                return qfalse;
//        }
//}
qboolean G_testvars( gentity_t *ent, int skiparg )
{     
   int i;
   for(i = 1; i <=49; i++ )
   {
	   trap_SendServerCommand( ent-g_entities, va(
        "print \"^5%d is %d.\n\"",i, ent->client->pers.badges[i] ) ); 
   }
   
   trap_SendServerCommand( ent-g_entities, va(
        "print \"^5This command its for testing porpuouses ignore it %d.\n\"",ent->client->pers.gameswin ) ); 
...
{     
  ...
  {
    trap_SendServerCommand( ent-g_entities, va(
    "print \"^7In order to use badges you need a ^5updated ^7your client\n^2Download from: ^5http://blogwtf.com/backport\n\"" ) );
    return qfalse;
  }
   trap_SendServerCommand( ent-g_entities, va(
        "print \"^3You have %d Badges of 43\n^3Your Stats: HTTP://BLOGWTF.COM/PLAYER/%d \n\"",
  ...
  if(ent->client->pers.badgetimer > level.time)
  {
   ...
   for(i = 1; i <=43; i++ )
   {
	   ...
	   switch(i)
		...
		break;
		case 9:
			ADMBP( "[I Bite Pretty Hard]\n" );
		break;
		case 10:
			ADMBP( "[Legendary Builder]\n" );
		break;
		case 11:
			ADMBP( "[Spitter]\n" );
		break;
		case 12:
			ADMBP( "[Legendary Destructor]\n" );
		break;
		case 13:
			ADMBP( "[Strategist Build]\n" );
		break;
		case 14:
			ADMBP( "[Gattling Gun]\n" );
		break;
		...
		break;
		case 21:
			ADMBP( "[Attack Granger]\n" );
		break;
		case 22:
			ADMBP( "[I like my eggs...scrambled]\n" );
		break;
		case 23:
			ADMBP( "[Flying Dragoon]\n" );
		break;
		case 24:
			ADMBP( "[Hugs and Swipings]\n" );
		break;
		case 25:
			ADMBP( "[Structure Killer]\n" );
		break;
		case 26:
			ADMBP( "[Entusiast]\n" );
		break;
		case 27:
			ADMBP( "[David and Goliath]\n" );
		break;
		case 29:
			ADMBP( "[Freezing Field]\n" );
		break;
		case 31:
			ADMBP( "[Red right hand-y weapon]\n" );
		break;
		case 32:
			ADMBP( "[Stealth Killer]\n" );
		break;
		case 33:
			ADMBP( "[Divine Gift]\n" );
		break;
		...
		break;
		case 35:
			ADMBP( "[Suicide Bomber]\n" );
		break;
		case 36:
			ADMBP( "[Poison Freak]\n" );
		break;
		case 37:
			ADMBP( "[Fire Breathing]\n" );
		break;
		case 38:
			ADMBP( "[Guardian Angel]\n" );
		break;
		case 39:
			ADMBP( "[Saver]\n" );
		break;
   ...
   ADMBP(va("\n^3%d Won Already\n^6Pending\n\n", ent->client->pers.badgesobtained ) );
   ADMBP(va("^3More Information: HTTP://BLOGWTF.COM/PLAYER/%d \n", ent->client->pers.mysqlid ) ); 
   ADMBP_end();
   return qtrue;
...
qboolean G_admin_forcespec( gentity_t *ent, int skiparg )
{
 ...
 if ( vic->client->pers.forcespec == qtrue )
 {
...
qboolean G_admin_unforcespec( gentity_t *ent, int skiparg )
{
 ...
 if ( vic->client->pers.forcespec == qfalse )
 {
...
vic->client->pers.forcespec = qfalse;
...
qboolean G_admin_drop( gentity_t *ent, int skiparg )
{
  ...
  if( G_SayArgc() < minargc )
  {
   trap_SendServerCommand( pids[ 0 ], va( "You have been dropped" ) );
  }
else
  {
  trap_SendServerCommand( pids[ 0 ], va( "disconnect \"You have been dropped.\n\n%s^7\n\"", msg ) );
  }
trap_DropClient( pids[ 0 ], va( "dropped" ) );
  return qtrue;
}
