...
vmCvar_t  g_globalpath;
vmCvar_t  g_specNoClip;

vmCvar_t g_reactorBuildTime;
vmCvar_t g_reactorHealRate;
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_suddenDeathTime, "g_suddenDeathTime", "50", CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },
...
{ &g_reactorBuildTime, "g_reactorBuildTime", "10000", CVAR_ARCHIVE, 0, qfalse },
{ &g_reactorHealRate, "g_reactorHealRate", "1", CVAR_ARCHIVE, 0, qfalse },
  ...
  { &g_globalpath, "g_globalpath", "/home/wserver/globals.global", CVAR_ARCHIVE,0, qfalse },
...
void G_CountSpawns( void )
{
  ...
  trap_SetConfigstring( CS_SPAWNS, va( "%d %d",
	...
	//Check lock rules
	if(level.numAlienSpawns <= 0 && level.alienTeamLocked == qfalse && (level.time - level.startTime) > 10000)
	{
		for( i = 1, ent = g_entities + i ; i < level.num_entities ; i++, ent++ )
		{
			if( !ent->client )
		...
		if( builders <= 0 || level.suddenDeath)
		{
			level.alienTeamLocked = qtrue;
			trap_SendServerCommand( -1, "print \"^2Alien Team Locked\n\"" );
		}
	}
	builders = 0;
	if(level.numHumanSpawns <= 0 && level.humanTeamLocked == qfalse && (level.time - level.startTime) > 10000)
	{
		for( i = 1, ent = g_entities + i ; i < level.num_entities ; i++, ent++ )
		{
			if( !ent->client )
		...
		{
			level.humanTeamLocked = qtrue;
			trap_SendServerCommand( -1, "print \"^2Human Team Locked\n\"" );
		}
	}
}
...
void CheckIntermissionExit( void )
{
  ...
  if( level.time > level.intermissiontime + 6000 )
  {
    ExitLevel( );
    return;
  }
  ...
  if( level.time < level.exitTime + 6000 )
    return;
...
==================
*/
static void G_activatewave( int turn )
{
	int i;
	gentity_t *ent;
	for( i = 0; i < level.maxclients; i++ )
	{
		...
		if(turn == 1)
		{
			if (ent->client->pers.teamSelection == PTE_ALIENS)
			{
				ent->client->pers.doublespeed = 1;
				G_AddEvent(ent, EV_MEDKIT_USED, 0);
				trap_SendServerCommand(ent - g_entities, va("print \"^1Overmind: ^3You are boosted.\n\""));

			}
			if (ent->client->pers.teamSelection == PTE_HUMANS)
			{
				ent->client->ps.persistant[PERS_CREDIT] += 2000;
				G_AddEvent(ent, EV_MEDKIT_USED, 0);
				trap_SendServerCommand(ent - g_entities, va("print \"^5Reactor: ^3Aliens boosted defend.\n\""));
			}
//			if(ent->client->pers.teamSelection == PTE_ALIENS)
//			{
//				ent->client->pers.doublehp = 1;
//				ent->client->pers.originalmaxhealth = ent->client->ps.stats[ STAT_MAX_HEALTH ];
//				ent->client->ps.stats[ STAT_MAX_HEALTH ] = ent->client->ps.stats[ STAT_MAX_HEALTH ] * 1.5;
//				if(ent->health > 0)
//				{
...
//				"print \"^1Overmind: ^3You are boosted.\n\"" ));
//			}
//			if(ent->client->pers.teamSelection == PTE_HUMANS)
//			{
//				if(level.devwarn < 3)
...
//			}
		...
		if(turn == 2)
		{
			if(ent->client->pers.teamSelection == PTE_ALIENS)
			{
				ent->client->ps.persistant[ PERS_CREDIT ] += 99;
				G_AddEvent( ent, EV_MEDKIT_USED, 0 );
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^1Overmind: ^3Humans boosted defend.\n\"" ));
			...
			if(ent->client->pers.teamSelection == PTE_HUMANS)
			{
				...
				G_AddEvent( ent, EV_MEDKIT_USED, 0 );
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^5Reactor: ^3You are boosted.\n\"" ));
			}
		}
	}
	level.waveactivated = 1;
}
static void G_deactivatewave( int turn )
{
	int i;
	gentity_t *ent;
	for( i = 0; i < level.maxclients; i++ )
	{
		...
		if( !ent->inuse ||
			...
			trap_SendServerCommand( ent-g_entities, va(
			"print \"^3Next wave in 5 minuts!!!\n\""));
			
			if(turn == 1)
			{
				if(ent->client->pers.teamSelection == PTE_ALIENS )
				{
					ent->client->pers.doublespeed = 0;
				}
				else if(ent->client->ps.persistant[ PERS_CREDIT ] > 2000)
				{
					ent->client->ps.persistant[ PERS_CREDIT ] = 1000;
				}
			}
			else
			{
				if(ent->client->pers.teamSelection == PTE_ALIENS )
				{
					if(ent->client->ps.persistant[ PERS_CREDIT ] > 90)
					{
	...
	level.waveactivated = 0;
}
static void G_Waves( void )
{
	int i,sec;
	gentity_t *ent;


	if(level.time300000 < 1)
	{
		level.time300000 = (level.time-level.startTime) + 300000;
		level.wavestage = 1;
	}
	if(level.time20000 < 1)
	{
		level.time20000 = (level.time-level.startTime) + 20000;
		level.waveminicounter = 0;
	}
	while(level.time300000 < (level.time-level.startTime))//Se ejecuta cada 5 minutos
	{
		level.time300000 = (level.time-level.startTime) + 300000;
		if(level.wavestage <= 4)
		{
			for( i = 0; i < level.maxclients; i++ )
			{
				ent = g_entities + i;
				...
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^1Overmind: ^3Evolving for wave %dp\n\"",25*level.wavestage ) );
				}
				else
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^5Reactor: ^3Upgrading for wave %dp\n\"",25*level.wavestage ) );
				}
			}
		}
		if(level.wavestage >= 4)
		{
			 //Turn off in the 20s loop.
			if((level.waveturn < 1))
			{
				if(level.alienStatsCounters.feeds > level.humanStatsCounters.feeds)
				{
					level.waveturn = 1;
				}
				else
			...
			G_activatewave( level.waveturn );
		}
		level.wavestage++;
	}
	//Write the turner off here.
	//Execute this every 20 seconds.
	while(level.time20000 < (level.time-level.startTime) && level.waveactivated == 1)//Se ejecuta cada 5 minutos
	{
		level.time20000 = (level.time-level.startTime) + 20000;

		sec = 60 - (level.waveminicounter * 20);
		if(level.waveturn == 1)
		{
			if(ent && ent->client && ent->client->pers.teamSelection == PTE_ALIENS)
			{
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^1Overmind: ^3You are boosted, attack the enemy base now Time Left: %d seconds.\n\"",sec ));
			}
			if(ent && ent->client && ent->client->pers.teamSelection == PTE_HUMANS)
			{
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^5Reactor: ^3Defend alien wave incoming: %d seconds left.\n\"", sec));
			}
		}
		else
		{
			...
			{
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^1Overmind: ^3Defend human wave incoming: %d seconds left.\n\"", sec ));
			}
			if(ent && ent->client && ent->client->pers.teamSelection == PTE_HUMANS)
			{
				trap_SendServerCommand( ent-g_entities, va(
				"print \"^5Reactor: ^3You are boosted, attack the enemy base now Time Left: %d seconds.\n\"",sec ));
			}
		}
		level.waveminicounter++;
		if(level.waveminicounter >= 5)
		{
			level.waveminicounter = 0;
			//Quitar poderes
			G_deactivatewave(level.waveturn);
		}
	}
	
	
	//Old code starts here.
	/*if(level.wavesactivated < 1 && level.wavewait < (level.time-level.startTime))
	{
		if((level.time-level.startTime) < (1000 * 60 * 5) && (level.time-level.startTime) > ((1000 * 60 * 5) -2000) && level.wavewarn1 < 1)
		{
			level.wavewarn1 = 1;
			level.wavewait = level.time-level.startTime + (1000*60*5);
		}	
		if((level.time-level.startTime) < (1000 * 60 * 10) && (level.time-level.startTime) > ((1000 * 60 * 10) -2000) && level.wavewarn2 < 1)
		{	
			level.wavewarn2 = 1;
			level.wavewait = level.time-level.startTime + (1000*60*5);
		}
			
		if((level.time-level.startTime) < (1000 * 60 * 15) && ((level.time-level.startTime) > ((1000 * 60 * 15) -2000)) && level.wavewarn3 < 1)
		{
			level.wavewarn3 = 1;
			level.wavewait = level.time-level.startTime + (1000*60*5);
		}
			
		if((level.time-level.startTime) < (1000 * 60 * 20) && (level.time-level.startTime) > ((1000 * 60 * 20) -2000) && level.wavewarn4 < 1)
		{
			level.wavewait = level.time-level.startTime + (1000*60*5);
			level.wavewarn4 = 1;
			level.wavesactivated = 1;
			level.wavetimer = level.time-level.startTime;
			level.timer20s = (level.time-level.startTime) + 20000;
			//Select the team who is gonna attack firsts.
			if((level.wavealienturn < 1 && level.wavehumanturn < 1))
			{
				if(level.alienStatsCounters.feeds > level.humanStatsCounters.feeds)
				{
					level.wavealienturn = 1;
					level.wavehumanturn = 0;
				}
				else
				{
					level.wavehumanturn = 1;
					level.wavealienturn = 0;
				}
			}
		}	
		for( i = 0; i < level.maxclients; i++ )
		{
			...
			if( !ent->inuse ||
				...
				( ent->client->pers.teamSelection != PTE_ALIENS && ent->client->pers.teamSelection != PTE_HUMANS ) )
				continue;
			if(level.wavewarn1 == 1 && level.wavewarn2 < 1)
			{
				//First message
				if(ent->client->pers.teamSelection == PTE_ALIENS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^1Overmind: ^3Evolving for wave 25p\n\"" ) );
					continue;
				}
				if(ent->client->pers.teamSelection == PTE_HUMANS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^5Reactor: ^3Upgrading for wave 25p\n\"" ) );
					continue;
				}
			}
			if(level.wavewarn2 == 1 && level.wavewarn3 < 1)
			{
				//Second message
				if(ent->client->pers.teamSelection == PTE_ALIENS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^1Overmind: ^3Evolving for wave 50p\n\"" ) );
					continue;
				}
				if(ent->client->pers.teamSelection == PTE_HUMANS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^5Reactor: ^3Upgrading for wave 50p\n\"" ) );
					continue;
				}
			}
			if(level.wavewarn3 == 1 && level.wavewarn4 < 1)
			{
				//Third message
				if(ent->client->pers.teamSelection == PTE_ALIENS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^1Overmind: ^3Evolving for wave 75p\n\"" ) );
					continue;
				}
				if(ent->client->pers.teamSelection == PTE_HUMANS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^5Reactor: ^3Upgrading for wave 75p\n\"" ) );
					continue;
				}
			}
			if(level.wavewarn4 == 1)
			{
				//Last message
				if(ent->client->pers.teamSelection == PTE_ALIENS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^1Overmind: ^3Evolve 100p done\n\"" ) );
				}
				if(ent->client->pers.teamSelection == PTE_HUMANS)
				{
					trap_SendServerCommand( ent-g_entities, va(
					"print \"^5Reactor: ^3Upgrade 100p done\n\"" ) );
				}
			}
		}
	}
	//Okay we have and attack and a defendor
	if((level.time-level.startTime) > (1000 * 60 * 20)) //This is kinda loop running all the time after the minut 20.
	{
		if((level.wavetimer + (1000 * 60)) > (level.time-level.startTime)) 
		{
			if(level.timer20s < level.time-level.startTime)
			{
				level.timer20s = level.time-level.startTime + 20000; //Perpetua este if asta que el superior salga
				for( i = 0; i < level.maxclients; i++ )
				{
					ent = g_entities + i;
					level.waveboosted++;
					...
					if(level.waveboosted < level.maxclients)
					{
						if(level.devwarn < 1)
						{
						...
						if(level.wavealienturn == 1)
						{
							if(ent->client->pers.teamSelection == PTE_ALIENS)
							{
								...
								ent->client->pers.doublehp = 1;
								ent->client->pers.originalmaxhealth = ent->client->ps.stats[ STAT_MAX_HEALTH ];
								ent->client->ps.stats[ STAT_MAX_HEALTH ] = ent->client->ps.stats[ STAT_MAX_HEALTH ] * 2;
								if(ent->health > 0)
								{
									ent->health = ent->client->ps.stats[ STAT_MAX_HEALTH ];
								}
						...
						if((level.waveboosted + 1) == level.maxclients)
						{
							trap_SendServerCommand( -1, va( "print \"" S_COLOR_BLUE "_\"" ) );
							level.wavetimer = level.time;
						}
					}
					sec = (((level.wavetimer + 1000 * 60) - level.time-level.startTime)/1000) % 60;
					if(level.wavealienturn == 1 && sec > 10)
					{
						if(ent->client->pers.teamSelection == PTE_ALIENS)
						{
							trap_SendServerCommand( ent-g_entities, va(
					...
					if(level.wavehumanturn == 1 && sec > 10)
					{
						if(ent->client->pers.teamSelection == PTE_ALIENS)
						{
							trap_SendServerCommand( ent-g_entities, va(
		...
		else //if((level.wavetimer + 1000 * 60) > (level.time-level.startTime))
		{
			//if((level.wavetimer + 1000 * 60) > level.time-level.startTime)
			//We arent rushing anymore Restore hp, evos and speed, and waveboosted.
			if(level.waveboosted > 0)
			{
				level.waveboosted--;
				for( i = 0; i < level.maxclients; i++ )
				{
					...
					if( !ent->inuse ||
						...
						if(ent->client->pers.teamSelection == PTE_ALIENS && level.wavealienturn == 1)
						{
								ent->client->pers.doublehp = 0;
								ent->client->ps.stats[ STAT_MAX_HEALTH ] = ent->client->ps.stats[ STAT_MAX_HEALTH ] / 2;
								ent->client->ps.stats[ STAT_MAX_HEALTH ] = ent->client->pers.originalmaxhealth;
								if(ent->health > 0)
								{
									ent->health = ent->client->ps.stats[ STAT_MAX_HEALTH ];
								}
						}
						if(ent->client->pers.teamSelection == PTE_HUMANS && level.wavealienturn == 1 )
						{
								ent->client->pers.trippleammo = 0;
								for( j = WP_NONE; j < WP_NUM_WEAPONS; j++ )
								{
									if( BG_InventoryContainsWeapon( j, ent->client->ps.stats ) )
									{
						...
						if(ent->client->pers.teamSelection == PTE_ALIENS && level.wavehumanturn == 1)
						{
							if(ent->client->ps.persistant[ PERS_CREDIT ] > 90)
							{
								ent->client->ps.persistant[ PERS_CREDIT ] -= 99;
						...
						if(ent->client->pers.teamSelection == PTE_HUMANS && level.wavehumanturn == 1)
						{
								ent->client->pers.doublespeed = 0;
						}
				}
			}
			//Check if are ready to the next attack and in case turn waveboosted to on, and flip the turns.
			if(level.waveboosted < level.maxclients && (level.wavewait < level.time-level.startTime)) //Es un multiplo de 5?
			{
				level.wavewait = (level.time-level.startTime) + (1000*60*5); //Get in this after 5 minuts.
				level.waveboosted = 0;
				level.wavetimer = (level.time-level.startTime);
				level.timer20s = (level.time-level.startTime) + 20000;
				if(level.wavehumanturn == 1)
