...
void SpawnCorpse( gentity_t *ent )
{
  ...
  return;
  //TODO:SPAWN CORPSES THAT CAN BE PICKED FOR RTD OR CREDITS OR W/E
  ...
  body->s.event = 0;
 // body->r.contents = CONTENTS_TRIGGER;
...
#define XGLOBAL_DENYBUILD	(1 << 0)
#define XGLOBAL_MUTE		(1 << 1)
#define XGLOBAL_FORCESPEC	(1 << 2)
#define XGLOBAL_BAN		(1 << 3)
#define XGLOBAL_WARNING		(1 << 4)
...
char *ClientConnect( int clientNum, qboolean firstTime )
{
  ...
  Q_strncpyz( guid, value, sizeof( guid ) );
	Q_strncpyz( guid2, value, sizeof( guid ) );
  ...
  if(trap_mysql_runquery( va("SELECT HIGH_PRIORITY id,credits,evos,timeplayed,adminlevel,playerlevel,gameswin,structsbuilt,structskilled,badges FROM players WHERE qkey = \"%s\" LIMIT 1", client->pers.guid)) == qtrue)
  {
		if( trap_mysql_fetchrow() == qtrue ) 
		{
				...
				trap_mysql_fetchfieldbyName( "credits", data, sizeof(data));
				client->pers.credits = atoi(data);
				trap_mysql_fetchfieldbyName( "evos", data, sizeof(data));
				client->pers.evos = atoi(data);
		...
		else
		{
			...
			if( trap_mysql_runquery( va("INSERT HIGH_PRIORITY INTO players (qkey,name,ip) VALUES (\"%s\",\"%s\",\"%s\")", client->pers.guid, client->pers.netname, client->pers.ip ) ) == qtrue )
			{
				trap_mysql_finishquery();
				...
				if(trap_mysql_runquery( va("SELECT HIGH_PRIORITY id FROM players WHERE qkey = \"%s\" LIMIT 1", client->pers.guid ) ) == qtrue)
				{
					if(trap_mysql_fetchrow() == qtrue)
					{
						...
						trap_mysql_finishquery();
						client->pers.mysqlid = atoi(data);
						client->pers.evos = 100;
						client->pers.credits = 10000;
...
void ClientBegin( int clientNum )
{
  ...
  client->pers.grenades = 0;
  client->pers.sawrow = 0;
  client->pers.mdadvgoonkills = 0;
  client->pers.tyrantkills = 0;
  client->pers.advmarakills = 0;
  client->pers.saweggstimer = 0;
  client->pers.saweggscounter = 0;
	client->pers.diehp = 0;
  client->pers.pouncekills = 0;
  client->pers.liskkills = 0;
  client->pers.stealthkills = 0;
  client->pers.godkills = 0;
  
  client->pers.trippleammo = 0;
  client->pers.doubleammo = 0;
  client->pers.doublehp = 0;
  client->pers.infiniteevos = 0;
  client->pers.doublespeed = 0;
  client->pers.droppedbombs = 0;
  client->pers.timerbombs = 0;
  
  client->pers.explosiontimer = 0;
  client->pers.explosionkills = 0;
  
  client->pers.flamerkills = 0;

  client->pers.blasterkills = 0;
  client->pers.energy = 0; //testing :p.
...
// This create a bug when there are over 20 players... the first players will be dropped due the flood of the msg
...
void ClientSpawn( gentity_t *ent, gentity_t *spawn, vec3_t origin, vec3_t angles )
{
  ...
  if(ent->client->pers.teamSelection == PTE_ALIENS && ent->client->pers.doublehp == 1)
  {
...
#define F_VEL   50.0f
  ...
  if(client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS || client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
  {
      level.lastspawntime = level.time;
...
//  if(client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS || client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS)
//  {
//       if(ent->client && ent->client->pers.doublehp < 1)
//		{
//			if(ent->client->pers.diehp > ent->client->ps.stats[ STAT_MAX_HEALTH ])
//			{
//				ent->client->ps.stats[ STAT_MAX_HEALTH ] = ent->client->pers.diehp;
//			}
//		}
//  }
}
...
void ClientDisconnect( int clientNum )
{
  ...
  if(ent->client->pers.mysqlid > 0)
  {
		...
		if( trap_mysql_runquery( va("UPDATE players SET credits=\"%d\",evos=\"%d\",timeplayed=\"%d\",adminlevel=\"%d\",lasttime=NOW() WHERE id=\"%d\" LIMIT 1", ent->client->pers.credits, ent->client->pers.evos, ent->client->pers.timeplayed, ent->client->pers.adminlevel, ent->client->pers.mysqlid ) ) == qtrue )
		{
		}
		else
		{
