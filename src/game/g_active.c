...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  client->time15000 += msec;
  client->time30000 += msec;
  ...
  {
      //This is supposed to run every second.
      //NOTE THIS RUNS EVERY SECOND ON EVERY CLIENT WARNING LOL
  ...
  while( client->time15000 >= 15000 )
  {
    client->time15000 -= 15000;
...
void ClientThink_real( gentity_t *ent )
{
  ...
  if (client->pers.rtdgodtime < level.time && (level.time > client->spawntime + 3000))
	{
		if( ( ent->flags & FL_GODMODE ) && !(ent->client->sess.spectatorState == SPECTATOR_FOLLOW) ){
		       ent->flags ^= FL_GODMODE;
		       /*if(ent->newbiebonus == 1)
						{
							ent->newbiebonus = 0;
							trap_SendServerCommand( ent-g_entities, va("print \"^3Newbie Bonus ^7OFF\n\"" ) );
						}
						else
						{*/
							trap_SendServerCommand( ent-g_entities, va("print \"^5You are no longer god\n\"" ) );
						/*}*/
		}
	}
	if (client->pers.rtdinvtime < level.time)
	{
		if( ( (ent->client->ps.eFlags  & EF_NODRAW) && !(ent->client->sess.spectatorState == SPECTATOR_FOLLOW)) ){
		       ent->client->ps.eFlags ^= EF_NODRAW;
					 trap_SendServerCommand( ent-g_entities, va("print \"^5You are no longer invisible\n\"" ) );
		}
	}
	if (ent->client->pers.rtdflytime < level.time)
	{
		if((ent->client->ps.stats[ STAT_STATE ] & SS_FLY) && !(ent->client->sess.spectatorState == SPECTATOR_FOLLOW))
		{
			ent->client->ps.stats[ STAT_STATE ] &= ~SS_FLY;
			trap_SendServerCommand( ent-g_entities, va("print \"^5Your Chicken Wings have been vanished\n\"" ) );
		}
	}
...
//    G_Printf("serverTime >>>>>\n" );
  ...
  {
     client->ps.speed = client->ps.speed * 1.2; // Was 1.5 but its the way to much
  }
  if(ent->client->pers.doublespeed == 1)
  {
	  client->ps.speed = client->ps.speed * 1.5; // LOL thats gonna be fast.
  }
  ...
  if( client->lastCreepSlowTime + CREEP_TIMEOUT < level.time )
    client->ps.stats[ STAT_STATE ] &= ~SS_ICEWAVED;
