...
static float PM_CmdScale( usercmd_t *cmd )
{
  ...
  {
        if( pm->ps->stats[ STAT_STATE ] & SS_ICEWAVED )
    {    
...
static qboolean PM_CheckJump( void )
{
  ...
  if( pm->ps->stats[ STAT_PTEAM ] == PTE_HUMANS )
    pm->ps->stats[ STAT_STAMINA ] -= 200; //500 per jump lets reduce it to 200
...
void PM_GlideMove(void)
{
    float scale,fmove,smove,wishspeed;
    vec3_t wishvel;
    int i;
    PM_Friction();
...
static void PM_Weapon( void )
{
  ...
  if( pm->ps->weaponstate == WEAPON_RELOADING )
  {
	...
	if( pm->ps->stats[ STAT_STATE ] & SS_TRIPPLEAMMO )
    {
		ammo*=3;
    }
  ...
  switch( pm->ps->weapon )
  {
    case WP_ALEVEL0:
      ...
      if( !pm->autoWeaponHit[ pm->ps->weapon ] && attack2 == qfalse)
      {
        pm->ps->weaponTime = 0;
        pm->ps->weaponstate = WEAPON_READY;
        return;
...
void PmoveSingle( pmove_t *pmove )
{
  ...
  else if((BG_ClassHasAbility(pm->ps->stats[STAT_PCLASS], SCA_FLYER)) && (pm->cmd.upmove >=10))
  {
        PM_WaterMove( );
        //PM_GlideMove();
  }
    //SHIT cant use ent here, need make a class...
   else if(pm->ps->stats[ STAT_STATE ] & SS_FLY && (pm->cmd.upmove >=10))
   {
       PM_WaterMove( );
       //PM_GlideMove();
    }
